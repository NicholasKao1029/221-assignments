...
class QTree {
 ...
 public:
  ...
  void helpWrite(Node *t, PNG & newImage);
 ...
 private:
  ...
  // push nw, ne, sw, se
  //split north south nbrs before west and east
   ...
   */
  void split(Node * t, bool bal);

  /** 
   * function that determines which node you are in relation to your parent
   *  0 if no parent 
   *  1 if nw
   *  2 if ne
   *  3 if sw 
  ...
  /**
   * Private helper function returns the number of leaves in a Qtree
   * @param t = ptr to a node. 
   */
  int numOfLeaf(Node *t);
  ...
  /** 
   * helper function to determine if neighbour cords can exist based on 
   * root node's dimensions. 
   */
  bool validNbr(Node * t, pair<int,int> nCord);
  Node *getNode (Node* ancestor, pair<int,int> nCord, int size);
  ...
  void createLeaves(Node * o, Node * root);
